...
void InstagramStream::serviceGrams(string url)
{
	...
	// This function loops until the app quits. Each iteration a pulls out the next result from the Instagram API query.
...
//				console() << nextQueryString << endl;
		...
		if( resultIt != searchResults.end() ) {
			try {
				string userName = (*resultIt)["caption"]["text"].getValue();
